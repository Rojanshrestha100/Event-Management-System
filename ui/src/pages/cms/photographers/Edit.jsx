...
const Edit = () => {
    ...
    useEffect(() => {
        http.get(`/cms/photographers/${params.id}`)
            .then(({ data }) => {
                setForm({
                    pcontactinfo: data.pcontactinfo,
                    content: data.content,
                })
                
                if(data.pimg) {
                    setUpImg(`${import.meta.env.VITE_API_URL}/images/${data.pimg}`)
                }
            })
            .catch(err => { })
    ...
    const handleSubmit = ev => {
        ...
        http.patch(`/cms/photographers/${params.id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            })
            .then(resp => navigate('/cms/photographers'))
            .catch(err => {})
    }
            ...
            <div className="row">
                <div className="col-5 mx-auto">
                    <form onSubmit={handleSubmit}>
                        <div className="mb-3">
                            <label htmlFor="name" className="form-label">Photographer Name</label>
                            <input type="text" name="pcontactinfo" id="pcontactinfo" className="form-control" required onChange={ev => setInForm(ev, form, setForm)} defaultValue={form.pcontactinfo} />
                        </div>
                        <div className="mb-3">
                        ...
                        <div className="mb-3">
                            ...
                            <div className="mt-3">{
                                pimg ? <img className="img-fluid" src={URL.createObjectURL(pimg)} /> : null
                            }</div>
                            <div className="mt-3">{
